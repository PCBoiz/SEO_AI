...
import { Check, Download, ExternalLink, Globe2, KeyRound, Loader2, PencilLine, Play, Square, UploadCloud } from "lucide-react";
import { Button } from "@/components/ui/button";
import { FormField, Input } from "@/components/ui/input";
import { XemTruocWeb } from "@/components/dung-web/xem-truoc-web";

/**
 * Thẻ "Website dựng sẵn" — XEM TRƯỚC, CHỐT, rồi mới ĐƯA LÊN MẠNG.
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * THỨ TỰ LÀ CHỦ ĐÍCH (chủ dự án, 18/09/2026): "chắc gì tạo bản mẫu phát là
 * người dùng ưng luôn". Bản trước mở thẻ là đập vào mắt "52 tệp, GitHub,
 * Cloudflare, npm install" — toàn việc SAU KHI ưng, mà chưa ai thấy trang.
 * Giờ: khung xem thử lên đầu (vẽ ngay trên máy chủ, không cần cài gì, chạy cả
 * trên Vercel — `lib/dung-web/ve-trang-tinh.ts`); dưới khung là hai nút "Ưng ý"
 * / "Chưa ưng — sửa"; phần số điện thoại, GitHub, .zip chỉ mở khi đã ưng.
 * Chi tiết kỹ thuật (trang, khối, ảnh, dữ liệu cần điền) gấp vào một mục.
 *
 * SỐ ĐIỆN THOẠI LÀ BẮT BUỘC KHI ĐƯA LÊN MẠNG, VÀ ĐÓ LÀ CHỦ ĐÍCH.
 *
...
export function DungWebCard({ projectId }: { projectId: string }) {
  ...
  /**
   * Đã ưng bản xem thử → mở phần đưa lên mạng. Mặc định mở sẵn khi đã lưu số
   * điện thoại: tức lần trước đã đi qua bước này (đẩy GitHub/tải .zip), không
   * bắt bấm "Ưng ý" lại mỗi lần mở trang.
   */
  ...
  return (
    <section id="dung-web" className={`glass flex flex-col gap-4 p-5 ${tt === null ? "min-h-[12rem]" : ""}`}>
      ...
      {tt === null ? (
        ...
        <>
          <p className="text-xs leading-relaxed text-muted-foreground">
            <strong className="text-foreground">{tt.tenWebsite}</strong> — {tt.soTrang} trang. Xem thử ngay bên dưới
            (bấm được các liên kết, đổi cỡ màn hình). Ưng rồi mới tới bước đưa lên mạng; chưa ưng thì bấm “Sửa”.
          </p>

          <XemTruocWeb
            projectId={projectId}
            trang={(tt.trang ?? []).map((t) => ({ duong: t.duong, tieuDe: t.tieuDe }))}
            ghiChu={tt.daLuu ? undefined : "Số điện thoại trên trang đang là số mẫu 0000 000 000 — điền số thật ở bước «Đưa lên mạng»."}
          />
          ...
          {daChot && (
            <div className="flex flex-col gap-4 border-t border-border/60 pt-4">
              <p className="text-xs leading-relaxed text-muted-foreground">
                <strong className="text-foreground">Đưa lên mạng.</strong> Điền số điện thoại thật (mọi nút gọi trên
                web dùng số này), rồi đẩy lên GitHub — Cloudflare tự dựng, miễn phí, cho phép thương mại. Hoặc tải
                .zip về cho người khác đưa lên.
              </p>
          ...
          <details className="group text-[11px] text-muted-foreground">
            <summary className="cursor-pointer select-none text-xs text-foreground/80 hover:text-foreground">
              Chi tiết bản dựng — {tt.soTrang} trang, {tt.soTep} tệp, ảnh, dữ liệu cần điền
            </summary>
            <div className="mt-2 flex flex-col gap-2 border-l border-border/60 pl-3">
              ...
              <p>
                Mã nguồn là một dự án Next.js: tải .zip rồi chạy <code className="metric">npm install</code> và{" "}
                <code className="metric">npm run dev</code> là chạy trên máy; các cách khác trong{" "}
