...
*Cập nhật lần cuối: 20/09/2026 — **A1 chạy lại một lần nữa**: bản vá 18/09 ĐÃ lên (tôi đo được), nhưng HSTS vẫn chưa bật vì Caddy giữ cấu hình cũ — `trien-khai.sh` giờ tự nạp lại; đợt này còn mang câu trả lời giá mới cho trợ lý AI trích. **A8 quan trọng hơn trước**: Trò chuyện giờ DỰNG ĐƯỢC website và cho xem thử ngay (19/09) — không có bảng thì không dùng được. 18/09: **A1 giờ KHẨN** (mang bản vá lỗ hổng nghiêm trọng của Next: thực thi mã từ xa qua bộ tối ưu ảnh — trang đang bật bộ đó); thêm **A9** Google Analytics. 16/09: **A2 đổi cách làm**: nhịp gõ lịch đăng giờ lấy từ cron của Vercel, chị chỉ cần đặt một biến môi trường (mục 24); không cần dán crontab lên VPS nữa. Thêm **A8** (chạy migration Neon cho màn Trò chuyện). Đợt kiểm trước khi gặp khách: xem `docs/kiem-truoc-khi-gap-khach-13-09.md`. Đây là **chỗ duy nhất** ghi việc
...
|---|---|---|---|---|
| **A1** | Trên VPS: `ssh root@103.7.40.145` → `cd /opt/halongxanh` → `./trien-khai.sh` | 3 phút | Đợt trước chị đã deploy (tôi đo trên trang thật sáng 13/09: đã có bản sửa 13/09). **Có 3 đợt sửa chưa lên**: (13/09) trang tin tức không bắt khách đợi 8 giây khi Neon ngủ; 3 trang accessibility 90–97 → 100; mô tả/tiêu đề SEO gọn lại. **(16/09)** trang tin tức hiện ngay tiêu đề thay vì màn trắng khi bộ đệm lạnh (đo FCP 14,0 s → 1,6 s); trang chủ bớt một ảnh hero tải thừa trên điện thoại (~112 KB). **(18/09 — KHẨN)** vá lỗ hổng nghiêm trọng của Next.js (GHSA-2xp9-vwfh-vxw4: kẻ lạ có thể chạy mã trên máy chủ qua bộ tối ưu ảnh, mà trang đang bật bộ đó) — Next 16.3.0 → 16.3.5, sharp → 0.35.4; bật HSTS 1 ngày (nấc đầu của thang); giấu `X-Powered-By`. Chưa deploy thì lỗ hổng còn nguyên trên trang thật. **(20/09 — chạy LẠI một lần)** đợt 18/09 đã lên (đo: có GA, không còn `X-Powered-By`), nhưng **HSTS vẫn không có** vì Caddy giữ cấu hình cũ trong bộ nhớ từ 27/08 — `trien-khai.sh` giờ tự bắt Caddy đọc lại. Đợt này cũng mang câu trả lời giá tự đứng được ("Giá Vinhomes Global Gate Hạ Long theo bảng hàng ngày …: từ … (liền kề) tới … (đơn lập)") để trợ lý AI trích. Xong nói tôi, tôi đo lại `Strict-Transport-Security: max-age=86400`. | `docs/kiem-truoc-khi-gap-khach-13-09.md` mục 1; `D:\vinhomes_ha_long_xanh\NHAT-KY.md` vòng 29–30 |
...
| **A7** | **Site ĐÃ xác minh trong Bing Webmaster (ảnh chị gửi 18/09) — tôi từng ghi "chưa xác minh" là SAI.** Việc còn lại: (1) **Sitemaps** → có dòng `https://halongxanh360.vn/sitemap.xml` chưa, trạng thái *Success* hay lỗi, cột "Discovered" bao nhiêu URL; chưa có thì *Submit sitemap*; (2) **URL Inspection** → dán `https://halongxanh360.vn/` → Bing nói gì (Discovered but not crawled / Crawled but not indexed / lỗi); (3) **Site Scan** → *Start new scan*. Chụp ba màn đó gửi tôi | 5 phút | Bing Webmaster hiện **0 hiển thị, 0 bấm** cho 9–16/09 và `site:` ra 0 trang: site có trong hệ thống nhưng Bing **chưa lập chỉ mục trang nào**. Kỹ thuật không chặn gì (robots mở, không noindex, canonical đúng). Với tên miền mới **không có liên kết trỏ tới** (Backlinks = 0) Bing rất dè dặt — nộp sitemap là bước đẩy rẻ nhất; URL Inspection sẽ nói thẳng lý do. | `NHAT-KY.md` halongxanh vòng 27 |
| **A8** | Trên máy chị, trong thư mục dự án: `npm run db:neon:migrate` — chạy **một lần** | 1 phút | Màn **Trò chuyện** (mới, 15/09) cần hai bảng mới trong Neon. Vercel tự dựng *mã* khi tôi đẩy, nhưng **không** tự tạo *bảng* — đó là việc chị giữ quyền, đúng như đã chốt. Chưa chạy thì `/tro-chuyen` trên mạng hiện dòng nhắc "chưa có bảng" thay vì lỗi 500; mọi màn khác không ảnh hưởng. **(19/09)** Trò chuyện giờ **dựng được website luôn** (trợ lý nhận lời → chạy 4 bước bằng khoá của chị → bản xem thử hiện ngay dưới tin, nút Ưng ý / Cần sửa) — tính năng này nằm trong màn Trò chuyện nên cũng chờ A8. | mục 23 |
| **A9** | Tạo Google Analytics 4 (analytics.google.com → Admin → Data streams → Web) → lấy Measurement ID dạng `G-…` → trên VPS mở `.env`, điền `NEXT_PUBLIC_GA_ID=G-…` → chạy `./trien-khai.sh` | 10 phút | **Đo 18/09: trang thật không có bộ đếm lượt truy cập nào.** Chị hỏi "sao chưa tiến triển" mà không có số lượt vào để nhìn — mọi bàn luận về lưu lượng đều là đoán. Mã đã sẵn (fail-closed: chưa điền thì trang không tải Google gì cả). Phải chạy lại `trien-khai.sh` vì biến này nhúng lúc dựng, không phải lúc chạy. | `NHAT-KY.md` halongxanh vòng 28 |
...
> **Vòng 89 (18/09) — bảo mật + đo lường**: `npm audit` ra lỗ hổng NGHIÊM TRỌNG
> của Next ở cả hai kho (thực thi mã từ xa qua bộ tối ưu ảnh); đã vá về 16.3.5,
> 0 lỗ hổng còn lại. **A1 giờ khẩn** vì trang thật vẫn chạy bản hở. Antigravity
> trên Vercel tự nhận bản vá khi tôi đẩy (chị kiểm A5). Thêm **A9**: Google
> Analytics — trang chưa có bộ đếm lượt truy cập nào.
>
