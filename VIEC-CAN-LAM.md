...
| **A7** | Vào **Bing Webmaster Tools** (bing.com/webmasters) → chọn `halongxanh360.vn`: site đã **xác minh** chưa? Mục *Sitemaps* đã có `https://halongxanh360.vn/sitemap.xml` chưa? Rồi dùng *URL Inspection* soi trang chủ | 5 phút | **Đo lại 18/09: Bing `site:halongxanh360.vn` vẫn 0 trang, DuckDuckGo cũng 0** — ba ngày không đổi. Kỹ thuật không chặn gì (robots mở, không noindex, canonical đúng — kiểm bằng UA Googlebot). IndexNow đã nhận 31 địa chỉ từ 11/09 nhưng đó là "báo có", không phải "cam kết lập chỉ mục"; chỗ kẹt là chưa xác minh site. Chưa xác minh thì chọn *Import from Google Search Console* (nhanh nhất). | `NHAT-KY.md` halongxanh vòng 27 |
